...
def filter_selected_kits(filter_session: Session, f_selected_kits):
    global ancestry_matchgroups, ancestry_matchtrees, ancestry_treedata, ancestry_icw, ancestry_ancestorcouple, \
    ...
    try:
        if ancestry_matchgroups:
            ancestry_matches = (
                filter_session.query(Ancestry_matchGroups)
                .filter(Ancestry_matchGroups.testGuid.in_(selected_guids))
                .all()
            )
            ...
            if ancestry_matchtrees:
                ...
                ancestry_matches_trees = (
                    filter_session.query(Ancestry_matchTrees)
                    .filter(Ancestry_matchTrees.matchid.in_(guids_to_check))
                    .all()
                )
                test_ids["Ancestry_matchTrees"] = [
                    match.Id for match in ancestry_matches_trees
                ]
        if ancestry_treedata:
            ancestry_tree_data = (
                filter_session.query(Ancestry_TreeData)
                .filter(Ancestry_TreeData.TestGuid.in_(selected_guids))
                .all()
            )
            test_ids["Ancestry_TreeData"] = [data.Id for data in ancestry_tree_data]
        if ancestry_icw:
            ancestry_icw_data = (
                filter_session.query(Ancestry_ICW)
                .filter(Ancestry_ICW.matchid.in_(selected_guids))
                .all()
            )
            test_ids["Ancestry_ICW"] = [icw.Id for icw in ancestry_icw_data]
        if ancestry_ancestorcouple:
            ancestry_ancestor_couple = (
                filter_session.query(AncestryAncestorCouple)
                .filter(AncestryAncestorCouple.TestGuid.in_(selected_guids))
                .all()
            )
            test_ids["AncestryAncestorCouple"] = [
                couple.Id for couple in ancestry_ancestor_couple
            ]
        if ancestry_matchethnicity:
            ancestry_match_ethnicity = (
                filter_session.query(Ancestry_matchEthnicity)
                .filter(Ancestry_matchEthnicity.matchGuid.in_(selected_guids))
                .all()
            )
            test_ids["Ancestry_matchEthnicity"] = [
                ethnicity.Id for ethnicity in ancestry_match_ethnicity
            ]
        if ftdna_matches2 or ftdna_chromo2 or ftdna_icw2 or dg_tree or dg_individual:
            if ftdna_matches2:
                ftdna_matches = (
                    filter_session.query(FTDNA_Matches2)
                    .filter(FTDNA_Matches2.eKit1.in_(selected_guids))
                    .all()
                )
                test_ids["FTDNA_Matches2"] = [match.Id for match in ftdna_matches]
            if ftdna_chromo2:
                ftdna_chromo = (
                    filter_session.query(FTDNA_Chromo2)
                    .filter(FTDNA_Chromo2.eKit1.in_(selected_guids))
                    .all()
                )
                test_ids["FTDNA_Chromo2"] = [chromo.Id for chromo in ftdna_chromo]
            if ftdna_icw2:
                ftdna_icw = (
                    filter_session.query(FTDNA_ICW2)
                    .filter(FTDNA_ICW2.eKitKit.in_(selected_guids))
                    .all()
                )
                test_ids["FTDNA_ICW2"] = [icw.Id for icw in ftdna_icw]
            if dg_tree:
                dg_trees = (
                    filter_session.query(DGTree)
                    .filter(DGTree.matchID.in_(selected_guids))
                    .all()
                )
                test_ids["DGTree"] = [tree.Id for tree in dg_trees]
            if dg_individual:
                dg_individuals = (
                    filter_session.query(DGIndividual)
                    .filter(DGIndividual.matchid.in_(selected_guids))
                    .all()
                )
                test_ids["DGIndividual"] = [
                    individual.Id for individual in dg_individuals
                ]
        if mh_match or mh_ancestors or mh_chromo or mh_icw or mh_tree:
            if mh_match:
                mh_matches = (
                    filter_session.query(MH_Match)
                    .filter(MH_Match.guid.in_(selected_guids))
                    .all()
                )
                test_ids["MH_Match"] = [match.Id for match in mh_matches]
            if mh_ancestors:
                mh_ancestors_data = (
                    filter_session.query(MH_Ancestors)
                    .filter(MH_Ancestors.matchid.in_(selected_guids))
                    .all()
                )
                test_ids["MH_Ancestors"] = [
                    ancestor.Id for ancestor in mh_ancestors_data
                ]
            if mh_chromo:
                mh_chromo_data = (
                    filter_session.query(MH_Chromo)
                    .filter(MH_Chromo.guid.in_(selected_guids))
                    .all()
                )
                test_ids["MH_Chromo"] = [chromo.Id for chromo in mh_chromo_data]
            if mh_icw:
                mh_icw_data = (
                    filter_session.query(MH_ICW)
                    .filter(MH_ICW.id1.in_(selected_guids))
                    .all()
                )
...
def insert_fact_type(fact_rm_session: Session):
    ...
    try:
        fact_type = (
            fact_rm_session.query(FactTypeTable).filter_by(Name="DNA Kit").first()
        )
        if not fact_type:
        ...
        logging.error(
            f"Error inserting or updating Fact Type 'DNA Kit' in FactTypeTable: {e}"
        )
        logging.error(traceback.format_exc())
...
def import_profiles(rm_session: Session, selected_kits):
    def get_gender_input(profile_name_given, profile_name_surname, profile_guid_value):
        while True:
            gender = (
                input(
                    f"Enter gender for kit {profile_name_given} {profile_name_surname}, "
                    f"GUID {profile_guid_value} ((M)ale, (F)emale, (U)nknown): "
                )
                .strip()
                .upper()
            )
    ...
    for kit_details in selected_kits:
        ...
        try:
            person_record = (
                rm_session.query(PersonTable)
                .filter_by(UniqueID=guid_value)
                .one_or_none()
            )
            if person_record:
                if person_record.Sex not in [0, 1]:
                    gender_value = get_gender_input(
                        name_given, name_surname, guid_value
                    )
                else:
                    gender_value = person_record.Sex
                if person_record.Sex != gender_value:
                    person_record.Sex = gender_value
                    person_record.UTCModDate = (
                            func.julianday(func.current_timestamp()) - 2415018.5
                    )
            else:
            ...
            existing_name = (
                rm_session.query(NameTable)
                .filter_by(OwnerID=person_id, NameType=6)
                .first()
            )
            if existing_name:
            ...
            logging.error(
                f"Error processing kit: {id_value}, GUID: {guid_value}, Name: {name_given} {name_surname}"
            )
            logging.error(traceback.format_exc())
            logging.exception(e)
            rm_session.rollback()
    ...
    try:
        ...
        if ancestry_matchgroups and filtered_ids.get("Ancestry_matchGroups"):
            ...
            def process_matchgroup(mg_session, group):
                match_tree = (
                    mg_session.query(Ancestry_matchTrees)
                    .filter_by(matchid=group.matchGuid)
                    .first()
                )
                if match_tree and match_tree.personId is not None:
                    person_id = hash_id(match_tree.personId, id_mapping)
                    father_id = (
                        hash_id(match_tree.fatherId, id_mapping)
                        if match_tree.fatherId is not None
                        else None
                    )
                    mother_id = (
                        hash_id(match_tree.motherId, id_mapping)
                        if match_tree.motherId is not None
                        else None
                    )
                    color = 18
                else:
                ...
                sex = 1 if subject_gender == "F" else 0 if subject_gender == "M" else 2
                return {
            ...
            match_groups = batch_limit(
                session,
                Ancestry_matchGroups,
                filtered_ids.get("Ancestry_matchGroups", []),
                lambda group: process_matchgroup(session, group),
                limit,
            )
        ...
        if ancestry_matchtrees and filtered_ids.get("Ancestry_matchTrees"):
            ...
            def process_matchtree(tree, person_data=None):
                try:
                    ...
                    father_id = (
                        hash_id(data_source.fatherId, id_mapping)
                        if data_source.fatherId is not None
                        else None
                    )
                    mother_id = (
                        hash_id(data_source.motherId, id_mapping)
                        if data_source.motherId is not None
                        else None
                    )
                    sex_value = 2
                    if data_source.personId is not None:
                        father_match = (
                            session.query(Ancestry_matchTrees)
                            .filter(
                    ...
                    if tree.relid == "1":
                        ...
                        father_id = (
                            hash_id(tree.fatherId, id_mapping)
                            if tree.fatherId is not None
                            else father_id
                        )
                        mother_id = (
                            hash_id(tree.motherId, id_mapping)
                            if tree.motherId is not None
                            else mother_id
                        )
                    else:
                        unique_id = generate_unique_id(
                            tree.given, tree.surname, tree.matchid, tree.relid
                        )
                    ...
                    logging.warning(
                        f"Invalid ID value for tree {tree.matchid}: {str(mte)}. Skipping."
                    )
                    return None
            ...
            try:
                match_trees = batch_limit(
                    session,
                    Ancestry_matchTrees,
                    filtered_ids.get("Ancestry_matchTrees", []),
                    process_matchtree,
                    limit,
                )
                ...
                logging.error(
                    f"An error occurred while processing Ancestry match trees: {str(e)}"
                )
        ...
        if ancestry_treedata and filtered_ids.get("Ancestry_TreeData"):
            ...
            def process_treedata(treedata):
                try:
                    ...
                    logging.warning(
                        f"Missing attribute in tree data: {str(tde)}. Skipping this record."
                    )
                    return None
            ...
            try:
                tree_data = batch_limit(
                    session,
                    Ancestry_TreeData,
                    filtered_ids.get("Ancestry_TreeData", []),
                    process_treedata,
                    limit,
                )
                ...
                logging.error(
                    f"An error occurred while processing Ancestry tree data: {str(e)}"
                )
                logging.exception("Exception details:")
                logging.error(traceback.format_exc())
        if ancestry_icw and filtered_ids.get("Ancestry_ICW"):
            ...
            def process_icw(icw):
                match_tree = (
                    session.query(Ancestry_matchTrees)
                    .filter_by(matchid=icw.matchid)
                    .first()
                )
                person_id = (
                    hash_id(match_tree.personId, id_mapping)
                    if match_tree and match_tree.personId is not None
                    else None
                )
                return {
            ...
            try:
                ...
                logging.error(
                    f"An error occurred while processing Ancestry ICW data: {str(e)}"
                )
                logging.exception("Exception details:")
                logging.error(traceback.format_exc())
        if ancestry_matchethnicity and filtered_ids.get("Ancestry_matchEthnicity"):
            ...
            def process_matchethnicity(ethnicity):
                try:
                    ...
                    logging.warning(
                        f"Missing attribute in match ethnicity: {str(mee)}. Skipping this record."
                    )
                    return None
            ...
            try:
                match_ethnicity_data = batch_limit(
                    session,
                    Ancestry_matchEthnicity,
                    filtered_ids.get("Ancestry_matchEthnicity", []),
                    process_matchethnicity,
                    limit,
                )
                processed_ancestry_data.extend(match_ethnicity_data or [])
            except Exception as e:
                logging.error(
                    f"An error occurred while processing Ancestry match ethnicity data: {str(e)}"
                )
                logging.exception("Exception details:")
                logging.error(traceback.format_exc())
        if ancestry_ancestorcouple and filtered_ids.get("AncestryAncestorCouple"):
            ...
            def process_ancestorcouple(couple):
                try:
                    ...
                    logging.warning(
                        f"Missing attribute in ancestor couple: {str(ace)}. Skipping this record."
                    )
                    return None
            ...
            try:
                ancestor_couple_data = batch_limit(
                    session,
                    AncestryAncestorCouple,
                    filtered_ids.get("AncestryAncestorCouple", []),
                    process_ancestorcouple,
                    limit,
                )
                processed_ancestry_data.extend(ancestor_couple_data or [])
            except Exception as e:
                logging.error(
                    f"An error occurred while processing Ancestry ancestor couple data: {str(e)}"
                )
                logging.exception("Exception details:")
    except Exception as e:
...
def process_ftdna(session: Session, filtered_ids):
    ...
    try:
        if ftdna_matches2 and filtered_ids.get("FTDNA_Matches2"):
            ...
            processed_ftdna_data.extend(
                batch_limit(
                    session,
                    FTDNA_Matches2,
                    filtered_ids["FTDNA_Matches2"],
                    process_ftdna_match,
                    limit,
                )
            )
        if ftdna_chromo2 and filtered_ids.get("FTDNA_Chromo2"):
            def process_ftdna_chromo(chromo):
                return {
                    "unique_id": generate_unique_id(
                        chromo.eKit1, chromo.eKit2, chromo.chromosome
                    ),
                    "color": 26,
                    "eKit1": chromo.eKit1,
                    "eKit2": chromo.eKit2,
            ...
            processed_ftdna_data.extend(
                batch_limit(
                    session,
                    FTDNA_Chromo2,
                    filtered_ids["FTDNA_Chromo2"],
                    process_ftdna_chromo,
                    limit,
                )
            )
        if ftdna_icw2 and filtered_ids.get("FTDNA_ICW2"):
            def process_ftdna_icw(icw):
                return {
                    "unique_id": generate_unique_id(
                        icw.eKitKit, icw.eKitMatch1, icw.eKitMatch2
                    ),
                    "color": 26,
                    "eKitKit": icw.eKitKit,
        ...
        if dg_tree and filtered_ids.get("DGTree"):
            ...
            processed_ftdna_data.extend(
                batch_limit(
                    session, DGTree, filtered_ids["DGTree"], process_dg_tree, limit
                )
            )
        if dg_individual and filtered_ids.get("DGIndividual"):
            def process_dg_individual(individual):
                return {
                    "unique_id": generate_unique_id(
                        individual.treeid, individual.matchid, individual.personId
                    ),
                    "sex": (
                        1
                        if individual.sex == "F"
                        else 0 if individual.sex == "M" else 2
                    ),
                    "color": 26,
            ...
            processed_ftdna_data.extend(
                batch_limit(
                    session,
                    DGIndividual,
                    filtered_ids["DGIndividual"],
                    process_dg_individual,
                    limit,
                )
            )
    except Exception as e:
    ...
    try:
        if mh_match and filtered_ids.get("MH_Match"):
            def process_mh_match(match):
                return {
                    ...
                    "sex": (
                        1 if match.gender == "F" else 0 if match.gender == "M" else 2
                    ),
                    "color": 27,
                    "name": match.name,
            ...
            processed_mh_data.extend(
                batch_limit(
                    session, MH_Match, filtered_ids["MH_Match"], process_mh_match, limit
                )
            )
        if mh_ancestors and filtered_ids.get("MH_Ancestors"):
            def process_mh_ancestors(ancestor):
                return {
                    ...
                    "sex": (
                        ...
                        else 0 if ancestor.gender == "M" else 2
                    ),
                    "color": 27,
            ...
            processed_mh_data.extend(
                batch_limit(
                    session,
                    MH_Ancestors,
                    filtered_ids["MH_Ancestors"],
                    process_mh_ancestors,
                    limit,
                )
            )
        if mh_chromo and filtered_ids.get("MH_Chromo"):
            def process_mh_chromo(chromo):
                return {
                    "unique_id": generate_unique_id(
                        chromo.guid, chromo.chromosome, chromo.start
                    ),
                    "color": 27,
                    "guid": chromo.guid,
                    "guid1": chromo.guid1,
        ...
        if mh_icw and filtered_ids.get("MH_ICW"):
            ...
            processed_mh_data.extend(
                batch_limit(
                    session, MH_ICW, filtered_ids["MH_ICW"], process_mh_icw, limit
                )
            )
        if mh_tree and filtered_ids.get("MH_Tree"):
            ...
            processed_mh_data.extend(
                batch_limit(
                    session, MH_Tree, filtered_ids["MH_Tree"], process_mh_tree, limit
                )
            )
    except Exception as e:
    ...
    try:
        ...
        for data in processed_data:
            ...
            if person_id is None and not unique_id:
                ...
                logging.warning(
                    f"Potentially problematic record: PersonID: None, UniqueID: None, sex: {sex_value}, relid: {relid}"
                )
                continue
            person_data = {
                "UniqueID": unique_id,
            ...
            if person_id is not None:
                existing_person = (
                    person_rm_session.query(PersonTable)
        ...
        logging.info(
            f"Processed {processed_count} person records. "
            f"{blank_record_count} records had neither PersonID nor UniqueID."
        )
    except Exception as e:
    ...
    try:
        ...
        for data in processed_data:
            ...
            if person_id is None:
                ...
                if unique_id:
                    person_record = (
                        name_rm_session.query(PersonTable)
                        .filter_by(UniqueID=unique_id)
                        .first()
                    )
                    if person_record:
                        ...
                        logging.warning(
                            f"No matching PersonID found for UniqueID: {unique_id}"
                        )
                else:
            ...
            if person_id is not None:
                existing_name = (
                    name_rm_session.query(NameTable)
                    .filter_by(OwnerID=person_id)
                    .first()
                )
            if existing_name:
...
def insert_family(family_rm_session: Session, processed_data, batch_size=limit):
    ...
    try:
        ...
        for data in processed_data:
            ...
            if family_id:
                existing_family = (
                    family_rm_session.query(FamilyTable)
                    .filter_by(FamilyID=family_id)
                    .first()
                )
            else:
                existing_family = (
            ...
            if existing_family:
                ...
                existing_family.UTCModDate = (
                        func.julianday(func.current_timestamp()) - 2415018.5
                )
                data["FamilyID"] = existing_family.FamilyID
            else:
                family_data = {
                    "FatherID": father_id,
                    "MotherID": mother_id,
            ...
            if father_id:
                father = (
                    family_rm_session.query(PersonTable)
                    .filter_by(PersonID=father_id)
                    .first()
                )
                if father:
    ...
    try:
        ...
        for data in processed_data:
            ...
            if not child_id or not family_id:
                logging.warning(f"Skipped processing record due to missing ChildID "
                                f"or FamilyID: ChildID={child_id}, FamilyID={family_id}"
                                )
                continue
            existing_child = (
                child_rm_session.query(ChildTable)
                .filter_by(ChildID=child_id, FamilyID=family_id)
                .first()
            )
            if existing_child:
                for key, value in data.items():
        ...
        for kit in selected_kits:
            ...
            for data in processed_data:
                if "source" in data and data["source"] == "process_matchgroup":
                    person_1 = (
                        dna_rm_session.query(PersonTable)
                        .filter_by(UniqueID=selected_kit_guid)
                        .first()
                    )
                    ...
                    dna_data = {
                        ...
                        "SharedCM": shared_cm,
                        "SharedPercent": (
                            round(shared_cm / 69, 2) if shared_cm else None
                        ),
                    ...
                    existing_dna = (
                        dna_rm_session.query(DNATable)
                        .filter_by(ID1=person_id_1, ID2=person_id_2)
                        .first()
                    )
                    if existing_dna:
                        for key, value in dna_data.items():
                            setattr(existing_dna, key, value)
                        existing_dna.UTCModDate = (
                                func.julianday(func.current_timestamp()) - 2415018.5
                        )
                    else:
...
def insert_events(event_rm_session: Session, processed_data, batch_size=limit):
    ...
    def transform_date(date_str):
        ...
        full_date_re = (
            "^(?P<day>\\d{1,2}) (?P<month>\\w{3,9}) (?P<year>-?\\d{1,4})(?: BC)?$"
        )
        month_year_re = "^(?P<month>\\w{3,9}) (?P<year>-?\\d{1,4})(?: BC)?$"
        year_only_re = "^(?P<year>-?\\d{1,4})(?: BC)?$"
        day_month_re = "^(?P<day>\\d{1,2}) (?P<month>\\w{3,9})$"
        month_only_re = "^(?P<month>\\w{3,9})$"
        between_years_re = ("^(T?between|bet) (?P<start_year>-?\\d{1,4})(?: BC)? "
                            "(and|-) (?P<end_year>-?\\d{1,4})(?: BC)?$")
        double_date_re = "^(?P<day>\\d{1,2}) (?P<month>\\w{3,9}) (?P<year>\\d{4})/(?P<alt_year>\\d{2})$"
        quaker_date_re = "^(?P<day>\\d{1,2})da (?P<month>\\d{1,2})mo (?P<year>\\d{4})$"
        qualifiers = {
            "abt": "A",
            "about": "A",
            "est": "E",
        ...
        def format_date(
                f_year, f_month, f_day, f_bc=False, double_date=False, quaker=False
        ):
        ...
        for data in processed_data:
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Invalid PersonID: {person_id}. Skipping this record."
                    )
                    continue
                place_ids = {}
                for place_type in ["birthplace", "deathplace"]:
                    ...
                    if place_name is None:
                        continue
                    existing_place = (
                        event_rm_session.query(PlaceTable)
                        .filter_by(Name=place_name)
                        .first()
                    )
                    if existing_place:
                        existing_place.UTCModDate = (
                                func.julianday(func.current_timestamp()) - 2415018.5
                        )
                ...
                for event_type in ["birthdate", "deathdate"]:
                    ...
                    existing_event = (
                        event_rm_session.query(EventTable)
                        .filter_by(
                            OwnerID=person_id, EventType=event_type_map[event_type]
                        )
                        .first()
                    )
                    if existing_event:
                        existing_event.Date = event_date
                        existing_event.UTCModDate = (
                                func.julianday(func.current_timestamp()) - 2415018.5
                        )
                    else:
...
def insert_group(group_rm_session: Session, processed_data, batch_size=limit):
    ...
    try:
        ...
        for data in processed_data:
            existing_group = (
                group_rm_session.query(GroupTable)
                .filter_by(GroupID=data["GroupID"])
                .first()
            )
            if existing_group:
                for key, value in data.items():
                    setattr(existing_group, key, value)
                existing_group.UTCModDate = (
                        func.julianday(func.current_timestamp()) - 2415018.5
                )
                logging.info(
                    f"Updated existing group record for GroupID: {data['GroupID']}"
                )
            else:
                group_data = {
                ...
                logging.info(
                    f"Inserted new group record for GroupID: {data['GroupID']}"
                )
            processed_count += 1
            if batch_size > 0 and processed_count % batch_size == 0:
...
def insert_url(url_rm_session: Session, processed_data, batch_size=limit):
    ...
    try:
        ...
        for data in processed_data:
            existing_url = (
                url_rm_session.query(URLTable)
                .filter_by(OwnerType=data["OwnerType"], OwnerID=data["OwnerID"])
                .first()
            )
            if existing_url:
                for key, value in data.items():
                    setattr(existing_url, key, value)
                existing_url.UTCModDate = (
                        func.julianday(func.current_timestamp()) - 2415018.5
                )
                logging.info(
                ...
                logging.info(
                    f"Inserted new URL record for OwnerType {data['OwnerType']} and OwnerID {data['OwnerID']}"
                )
            processed_count += 1
            if batch_size > 0 and processed_count % batch_size == 0:
...
def rebuild_all_indexes(engine):
    ...
    tables_indexes = [
        ...
        (
            FactTypeTable,
            ["idxFactTypeName", "idxFactTypeAbbrev", "idxFactTypeGedcomTag"],
        ),
    ...
    with engine.begin() as conn:
        for table, index_names in tables_indexes:
            ...
            for index_name in index_names:
                index = next(
                    (idx for idx in table.__table__.indexes if idx.name == index_name),
                    None,
                )
                if index:
                    ...
                    print(
                        f"Warning: Index {index_name} not found for table {table_name}"
                    )
            if engine.dialect.name != "sqlite":
