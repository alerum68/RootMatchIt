...
# DNAGEDCOM_DB_PATH = r"C:\DNAGEDcom.db"
# ROOTSMAGIC_DB_PATH = r"C:\RootsMagic.rmtree"
...
def user_kit_data(conn_dg):
    ...
    try:
        ...
        logging.info("Extracted user kit data successfully.")
    except sqlite3.Error as e:
...
def insert_or_update_dna_kit_fact_type(cursor_rm):
    ...
    try:
        cursor_rm.execute("""
            INSERT OR REPLACE INTO FactTypeTable (FactTypeID, Name, UseValue, Abbrev, GedcomTag, Sentence)
            VALUES (
                (SELECT FactTypeID FROM FactTypeTable WHERE Name = 'DNA Kit'),
                'DNA Kit', 1, 'DNA_Kit', 'EVEN', '[person] had a DNA test performed. Kit #:< [Desc]>.'
            )
        """)
        logging.info("Inserted or updated 'DNA Kit' in FactTypeTable.")
    except sqlite3.Error as e:
        logging.error(f"Error inserting or updating 'DNA Kit' in FactTypeTable: {e}")


def process_and_insert_user_data(conn_rm, dna_kits):
    # Process and insert user data into RootsMagic database
    cursor_rm = None
    try:
...
def ancestry_import(conn_dg, conn_rm):
    # Fetch and process data from Ancestry tables in DNAGedcom
    cursor_dg = None
    cursor_rm = None
    try:
        cursor_dg = conn_dg.cursor()
        ...
        logging.error(f"Error importing data from Ancestry tables: {e}")
    finally:
        if cursor_dg:
            cursor_dg.close()
        if cursor_rm:
